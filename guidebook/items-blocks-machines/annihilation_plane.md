...
毁灭面板只有在网络能够存储其掉落物/物品时，才会破坏方块或拾取物品。这意味着要对其进行过滤，*你必须限制其网络能存储的内容*，最常用的方法是将它放在[子网络](/ae2-mechanics/subnetworks.md)上。可以使用<ItemLink id="storage_bus" />或[存储元件](/items-blocks-machines/storage-cells.md)进行[分区](cell-workbench.md)来实现这一点。
