...
## [下载页](https://github.com/Jerry114514/Applied-Energistics-2-Guide-book-in-Simplified-Chinese/releases)
...
* 若您要提出关于翻译或本地化的内容的错误或建议，请通过Issues，邮箱或b站私信联系我。

* 感谢Lobster Agent项目（参见末尾），使我这样的普通人也有了本地Agent的使用机会，也使得翻译-编译工作流的实现与落地，最终催生了 `0.0.1 Alpha 预览版` 的发布与后续更新。

> 本项目翻译有部分AI翻译，仅供参考和学习。因翻译错误造成的损失概不负责。
> 包括但不限于：游戏内物品，存档，数据等。
...
* Q：我可以直接把这些文件替换原本的文件并打包成模组吗？
  A：不可以！本项目内的文件是为了方便直接打开阅读而经过修改的，后续会单独推出可以直接替换文件的版本。

  直接替换jar文件中的内容会破坏文件，而直接替换并编译会报错。
  
* Q：为何不直接在Crowdin上参与汉化？
  A：因为指南书的文本并未在列。
* Q: 为什么有时候能看见句子中掺杂英文，比如：`这个容器就是分子装配室ae2:molecular_assembler。`
  A: 这是该物品的命名空间ID，为了方便玩家查表，或通过/give指令获取该物品。可以通过在源码模式/编辑模式下阅读解决这个问题。
